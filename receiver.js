...
const { amqpURI, host, connectOptions,
    ...
    cacheStrategy, cachePath, retryQueue } = config.getConfig();
    ...
    try {
        ...
        if (jobType === 'clear') {
            ...
            const job = `jobId: ${buildConfig.jobId}, ` +
                `jobType: ${jobType}, buildId: ${buildId}`;
                    ...
                    if (retryCount >= messageReprocessLimit) {
                        ...
                        helper.updateBuildStatus(
                            ...
                            (err, response) => {
                                if (err) {
                                    // eslint-disable-next-line max-len
                                    logger.error(`failed to update build status for build ${buildId}: ${err} ${JSON.stringify(response)}`);
                                } else {
                                    // eslint-disable-next-line max-len
...
const onRetryMessage = async (data) => {
    try {
        ...
        logger.error(`${retryQueue}: error ${err}, acknowledge ` +
...
const listen = async () => {
    ...
    const setup = channel => Promise.all([
